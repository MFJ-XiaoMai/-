<template>
    <div class="aside">
        ...
        <div class="history">
            <h5>历史查询</h5>
            <nuxt-link :to="`/air/flights?departCity=${item.departCity}&departCode=${item.departCode}&destCity=${item.destCity}&
            destCode=${item.destCode}&departDate=${item.departDate}`"
            v-for="(item,index) in airsHistory"
            :key='index'>
                <el-row type="flex" 
...
export default {
  ...
  mounted(){
    //获取历史记录
    this.airsHistory = JSON.parse(localStorage.getItem('airs') || `[]`)  }
}
</script>
